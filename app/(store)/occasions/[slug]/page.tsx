import { occasions, products } from '@/lib/data/mock';
...
export default async function OccasionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Find products that match this occasion
  const occasionProducts = products.filter(p => p.occasion.includes(occasion.name));
    ...
    <div className="bg-cream min-h-screen pb-20">
      ...
      <div className="relative h-[30vh] md:h-[40vh] w-full bg-charcoal">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-white/90 text-lg md:text-xl max-w-2xl drop-shadow-sm">
            Curated elegance for your special {occasion.name.toLowerCase()} moments.
          </p>
        </div>
      </div>
      ...
      <Container className="mt-8">
        ...
        <div className="flex items-center text-sm text-muted-foreground mb-8 overflow-x-auto pb-2 whitespace-nowrap">
          ...
          <Link href="/occasions" className="hover:text-charcoal transition-colors cursor-default">Occasions</Link>
          <ChevronRight className="w-4 h-4 mx-2 shrink-0" />
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
              <p className="text-muted-foreground text-sm">
                Showing <span className="font-medium text-charcoal">{occasionProducts.length}</span> products
              </p>
            ...
            {occasionProducts.length > 0 ? (
              ...
              <div className="text-center py-20 bg-white border border-border-light border-dashed rounded-lg">
                ...
                <p className="text-muted-foreground">We couldn't find any products for this occasion yet.</p>
              </div>
            )}
          </div>
        </div>
